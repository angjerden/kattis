...
The doctor’s instructions are to say “aaah”.
Unfortunately, the doctors sometimes need Jon Marius to say “aaah” for a while,
which Jon Marius has never been good at.
Each doctor requires a certain level of “aah” – some require “aaaaaah”,
while others can actually diagnose his throat with just an “h”.
...
if __name__ == "__main__":
    for filename in sys.argv[1:]:
        ...
        jon = file.readline().rstrip()
        doctor = file.readline().rstrip()

        #check that input only consists of the characters 'a' and 'h'
        for word in [jon, doctor]:
            for char in word:
                if char not in ['a', 'h']:
                    fail()

        #check that last character is an 'h'
        if jon[-1] != 'h' or doctor[-1] != 'h':
            fail()
